...
/**
 * Export single medication profile to PDF
 * GET /api/medications/:id/export/pdf
 */
async function exportMedicationDetailPdfHandler(req, res) {
  try {
    if (!req.user) {
      return res.status(401).json({
        success: false,
    ...
    const pdfBuffer = await exportMedicationDetailToPdf(req.user, id);

    res.setHeader("Content-Type", "application/pdf");
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="medication-${String(id).slice(0, 8)}-${Date.now()}.pdf"`
    );
    res.send(pdfBuffer);
  } catch (err) {
    console.error("Export medication detail PDF error:", err);
    const msg = err.message || "";
    const statusCode =
      msg.includes("not found") || msg.includes("access denied") ? 404 : 500;
    return res.status(statusCode).json({
      success: false,
      message: msg || "Failed to export medication PDF",
    });
  }
}
...
module.exports = {
  ...
  exportMedicationDetailPdf: exportMedicationDetailPdfHandler,
