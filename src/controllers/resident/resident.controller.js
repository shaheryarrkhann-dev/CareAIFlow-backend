...
async function deleteResidentHandler(req, res) {
  try {
    ...
    try {
      await createAuditLog({
        action: "PHI_DELETE_RESIDENT",
        userId: req.user.id,
        tenantId: resident.tenantId,
        resource: "resident",
        resourceId: resident.id,
        description: "Resident soft deleted",
        method: req.method,
        endpoint: req.originalUrl || req.path,
        req,
        metadata: {
      ...
      console.error("Failed to log audit for resident deletion:", {
        message: auditError?.message,
        stack: auditError?.stack,
        error: auditError,
      });
...
async function updateResidentHandler(req, res) {
  try {
    ...
    if (req.file) {
      try {
        const photoUrl = await uploadResidentPhotoToS3(
          req.file,
          updatedResident.id,
          tenantId
        );
        ...
        const residentWithPhoto = await prisma.resident.update({
          ...
          data: {
            residentPhoto: photoUrl,
            residentIdentificationProfilePicture: photoUrl,
          },
        });
