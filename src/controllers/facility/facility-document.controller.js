...
exports.uploadDocument = async (req, res, next) => {
  try {
    ...
    const documentCompliance = await afterFacilityDocumentChange(
      tenantId,
      doc.facilityId,
      req.user,
      { req }
...
exports.updateDocument = async (req, res, next) => {
  try {
    ...
    const facilityId = resolveFacilityId(req);
    const doc = await facilityDocumentService.updateDocument(
      tenantId,
      req.params.id,
      req.body,
      facilityId
    ...
    try {
      await createAuditLog({
        userId: req.user.id,
        tenantId,
        action: "FACILITY_DOCUMENT_MODIFIED",
        resource: "facility_document",
        resourceId: doc.id,
        description: "Facility document metadata updated",
        method: req.method,
        endpoint: req.originalUrl || req.path,
        req,
        metadata: { fileName: doc.fileName, facilityId: doc.facilityId },
