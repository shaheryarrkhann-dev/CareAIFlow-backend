...
async function deleteExtractionHandler(req, res) {
  try {
    ...
    // Delete extraction (includes S3 file cleanup)
    const deletedExtraction = await deleteNcpExtraction(extractionId, tenantId);

    return res.status(200).json({
      success: true,
      message: "Extraction deleted successfully",
      extraction: {
        id: deletedExtraction.id,
        sourcePdfFileName: deletedExtraction.sourcePdfFileName,
    ...
    return res.status(500).json({
      success: false,
      message: error.message || "Failed to delete extraction",
    });
  }
}
