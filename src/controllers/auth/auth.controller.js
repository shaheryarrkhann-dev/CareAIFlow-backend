 ...
 * Self-serve account creation for subscription onboarding
 */
 ...
 */
exports.emailVerificationStatus = async (req, res, next) => {
  try {
    handleValidation(req);
...
exports.inviteUser = async (req, res, next) => {
  try {
    ...
    if (
      msg.includes("cannot invite yourself") ||
 ...
 * Public: preview org/role for accept-invite page (does not consume token)
 */
exports.getInvitePreview = async (req, res, next) => {
  try {
    ...
    const result = await authService.getInvitePreview(token);
    return res.json(result);
  } catch (err) {
    if (err.code) err.status = err.status || 400;
    next(err);
  }
};
 ...
 * Public: accept invite, set password, return session
 */
