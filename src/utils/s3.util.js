 ...
 * @param {Object} params - { userId, buffer, mimeType, fileName }
 * @returns {Promise<Object>} { s3Key, s3Url, success }
 */
async function uploadUserAvatarToS3({ userId, buffer, mimeType, fileName }) {
  const allowedMime = ["image/jpeg", "image/jpg", "image/png", "image/gif", "image/webp"];
  if (!allowedMime.includes((mimeType || "").toLowerCase())) {
    throw new Error("Avatar must be JPEG, PNG, GIF, or WebP");
  }
  const timestamp = Date.now();
  const ext = (fileName || "avatar").split(".").pop()?.toLowerCase() || "png";
  const safeExt = ["png", "jpg", "jpeg", "gif", "webp"].includes(ext) ? ext : "png";
  const s3Key = `users/${userId}/avatar/${timestamp}.${safeExt}`;
  ...
  const params = {
    ...
    ContentType: mimeType,
    ServerSideEncryption: "AES256",
    Metadata: { userId, uploadedAt: new Date().toISOString(), type: "user-avatar" },
  };

  await s3Client.send(new PutObjectCommand(params));
  const s3Url = buildPublicS3ObjectUrl(s3Key);
  console.log(`[S3] Uploaded user avatar: ${s3Key}`);
  return { success: true, s3Key, s3Url };
}

/**
 * Store an outbound email/fax PDF attachment for dashboard viewing.
 */
async function uploadOutboundDeliveryPdfToS3({
  tenantId,
  fileName,
  buffer,
}) {
  const timestamp = Date.now();
  const sanitizedFileName = String(fileName || "report.pdf").replace(
    /[^a-zA-Z0-9._-]/g,
    "_"
  );
  const safeName = sanitizedFileName.toLowerCase().endsWith(".pdf")
    ? sanitizedFileName
    : `${sanitizedFileName}.pdf`;
  const s3Key = `${tenantId}/outbound-deliveries/${timestamp}_${safeName}`;
  ...
  const params = {
    ...
    Metadata: {
      ...
      uploadedAt: new Date().toISOString(),
      type: "outbound-delivery-attachment",
    },
  };

  await s3Client.send(new PutObjectCommand(params));
  const s3Url = buildPublicS3ObjectUrl(s3Key);
  console.log(`[S3] Uploaded outbound delivery attachment: ${s3Key}`);
  return { success: true, s3Key, s3Url };
}
...
module.exports = {
  ...
  uploadUserAvatarToS3,
  uploadOutboundDeliveryPdfToS3,
