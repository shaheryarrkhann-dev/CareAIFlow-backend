 ...
 * @param {{ userId?: string, userRole?: string }} [opts] - Billing user for slot checks
 * @returns {Promise<Object>} Created facility
 */
async function createFacility(tenantId, data, opts = {}) {
  if (!tenantId) throw new Error("tenantId is required");
  const tenant = await prisma.tenant.findUnique({
    where: { id: tenantId },
    select: { id: true },
  ...
  if (opts.userId) {
    await subscriptionManagement.assertCanAddFacility(opts.userId, {
  ...
  // Double-submit / retry guard: identical name created in last 60s
 ...
 * @param {{ userRole?: string }} [opts]
 * @returns {Promise<Object>} Updated facility record
 */
async function updateFacility(tenantId, facilityId, data, opts = {}) {
  if (!tenantId) {
    throw new Error("tenantId is required");
  }
  ...
  if (letterheadConfig !== undefined && letterheadConfig !== null) {
    updateData.letterheadConfig = letterheadConfig;
  }

  if (residentCapacityLimit !== undefined) {
    if (opts.userRole !== "SUPER_ADMIN") {
      throw Object.assign(
        new Error(
          "Only a Super Admin can change the resident capacity limit for a facility",
        ),
        { status: 403 },
      );
    }
    if (!isAllowedResidentCapacityLimit(residentCapacityLimit)) {
      throw Object.assign(
        new Error("Resident capacity limit must be 6, 7, or 8"),
        { status: 400 },
      );
    }
    updateData.residentCapacityLimit = normalizeResidentCapacityLimit(
      residentCapacityLimit,
    );
  }
...
async function createIndependentFacility(userId, userRole, data) {
  ...
  await subscriptionManagement.assertCanAddFacility(userId, { userRole });
...
/**
 * Super Admin only: set residentCapacityLimit (6|7|8) on selected facilities
 * in a tenant, or all facilities when facilityIds is empty/omitted.
 * @param {{ tenantId: string, facilityIds?: string[], residentCapacityLimit: number, userRole?: string }} params
 */
async function bulkSetResidentCapacityLimit({
  tenantId,
  facilityIds,
  residentCapacityLimit,
  userRole,
  ...
  if (userRole !== "SUPER_ADMIN") {
    throw Object.assign(
      new Error(
        "Only a Super Admin can change resident capacity limits for facilities",
      ),
      { status: 403 },
    );
  }
  ...
  const limit = normalizeResidentCapacityLimit(residentCapacityLimit);
  const tenant = await prisma.tenant.findUnique({
    where: { id: tenantId },
    select: { id: true },
  });
  ...
  const ids = Array.isArray(facilityIds)
    ? facilityIds.filter(Boolean)
    : [];
  ...
  if (ids.length > 0) {
    ...
    if (owned.length !== ids.length) {
      throw Object.assign(
        new Error(
          "One or more facilities were not found in this organization",
        ),
        { status: 400 },
      );
    }
  ...
  const result = await prisma.facility.updateMany({
    where,
    data: { residentCapacityLimit: limit },
  });
  ...
  return { updatedCount: result.count, facilities, residentCapacityLimit: limit };
}
