...
/**
 * Permanently delete a drill (tenant facility scope). Admin/Super Admin only at route layer.
 * @param {string} tenantId
 * @param {string} drillId
 * @param {string} [facilityId]
 * @returns {Promise<object|null>} Deleted drill row or null if not found
 */
async function deleteDrill(tenantId, drillId, facilityId) {
  const resolvedFacilityId = await resolveFacilityId(tenantId, facilityId);
  const drill = await prisma.evacuationDrill.findFirst({
    where: { id: drillId, facilityId: resolvedFacilityId },
  });
  if (!drill) return null;
  await prisma.evacuationDrill.delete({ where: { id: drillId } });
  return drill;
}
