...
/**
 * @returns {{ action: "none" | "deleted" | "deactivated", assignmentCount?: number }}
 */
async function retireAdminSystemTemplate() {
  ...
  const assignmentCount = adminTemplate._count.userRoleAssignments;
  ...
  if (assignmentCount > 0) {
    ...
    return {
      ...
      assignmentCount,
      message:
        "Legacy Admin custom template hidden from dropdown (users still assigned — org admin uses invite role ADMIN).",
    };
  }
  ...
  return {
    action: "deleted",
    message: 'Removed deprecated "Admin" system template from database.',
  };
}
...
async function seedPermissionsAndTemplates(options = {}) {
  ...
  try {
    ...
    result.roleTemplatesSeeded = SYSTEM_ROLE_TEMPLATES.map((t) => t.name);
    result.deprecatedAdminTemplate = await retireAdminSystemTemplate();
    result.excludesAdminCustomRole = true;
