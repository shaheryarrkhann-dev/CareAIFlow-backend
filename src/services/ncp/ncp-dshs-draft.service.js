...
// ─────────────────────────────────────────────────────────────────────────────
// Post-processing: enforce caregiver subject consistency in code
// ─────────────────────────────────────────────────────────────────────────────

// Fields that must start with "Staff will" and have no other subjects.
// Use endsWith (not substring) to avoid matching checkbox fields like mobility_room_assistance.
const IS_CAREGIVER_FIELD = (name) =>
  name.endsWith("_caregiver_actions") ||
  name.endsWith("_assistance") ||
  name.startsWith("caregiver_");

// Wrong subject pattern — "Caregiver will/to" is now wrong; "Staff will" is the standard
const ANY_WRONG_SUBJECT = /\b(?:\w+\s+)?(?:caregiver[s]?)\s+(?:members?\s+)?(?:will|to|must|should)\s+/gi;
// Mid-SEMICOLON repeat only: "X; Staff will Y" → "X; Y" (never after "." — that starts a new sentence)
const MID_SEMICOLON_REPEAT = /;\s*Staff\s+(?:will|to)\s+/gi;
// "Staff will" followed by any wrong subject phrase (caregiver, they, family member, etc.)
const DOUBLE_START = /^(Staff\s+(?:will|to)\s+)(?:(?:the\s+)?(?:caregiver[s]?|nurse[s]?|family\s+members?|they|he|she)\b[^,;.]{0,30}?(?:will|to|must|should|can|shall)\s+)/i;
// Duplicate "Staff will" at the very start — AI echoes the prefix from the raw value
const DUPLICATE_STAFF_START = /^(Staff\s+(?:will|to)\s+)(?:Staff\s+(?:will|to)\s+)+/gi;

/**
 * Code-level cleanup applied after every AI layer — catches what the prompt cannot guarantee.
 *
 * For caregiver action / assistance fields:
 *   - "Staff will Staff will..."               → "Staff will ..."
 *   - "Caregiver will..."                      → "Staff will ..."
 *   - "Caregivers will..."                     → "Staff will ..."
 *   - "X; Staff will Y"                        → "X; Y"
 *   - "Staff will" alone (empty result)        → cleared
 *
 * For ALL fields:
 *   - "Caregiver will" / "Caregivers will" anywhere → "Staff will"
 */
function cleanCaregiverFields(data, schema = {}) {
  let fixedCount = 0;
  let caregiverWillCount = 0;
  let duplicateStaffCount = 0;

  for (const [key, value] of Object.entries(data)) {
    if (typeof value !== "string" || !value.trim()) continue;

    // Skip checkbox/passthrough fields — they must never be prefixed
    if (isPassthrough(key, schema[key])) continue;

    let v = value.trim();

    // Skip short values — enum selections, single words, not narrative text
    if (IS_CAREGIVER_FIELD(key) && v.length < 25) continue;

    const original = v;

    if (IS_CAREGIVER_FIELD(key)) {
      // 1. Strip wrong subject immediately after "Staff will "
      v = v.replace(DOUBLE_START, "$1");

      // 2. Replace any wrong subject at the very start → "Staff will "
      if (/^(?:(?:\w+\s+)*(?:caregiver[s]?)\s+(?:members?\s+)?(?:will|to|must|should)\s+)/i.test(v)) {
        console.log(`[NCP-DSHS-CLEAN] ⚠️  Field "${key}": starts with "Caregiver will" — replacing with "Staff will"`);
        caregiverWillCount++;
        v = v.replace(/^(?:(?:\w+\s+)*(?:caregiver[s]?)\s+(?:members?\s+)?(?:will|to|must|should)\s+)/i, "Staff will ");
      }

      // 2b. Collapse "Staff will Staff will ..." created by step 2
      if (DUPLICATE_STAFF_START.test(v)) {
        console.log(`[NCP-DSHS-CLEAN] ⚠️  Field "${key}": duplicate "Staff will Staff will" detected — collapsing`);
        duplicateStaffCount++;
        DUPLICATE_STAFF_START.lastIndex = 0;
        v = v.replace(DUPLICATE_STAFF_START, "$1");
      }

      // 3. Ensure starts with "Staff will" (if AI wrote something else entirely)
      if (!/^Staff\s+(?:will|to)\s+/i.test(v) && v.length > 0) {
        console.log(`[NCP-DSHS-CLEAN] ⚠️  Field "${key}": does not start with "Staff will" — prepending`);
        v = "Staff will " + v.charAt(0).toLowerCase() + v.slice(1);
      }

      // 4. Strip repeated "Staff will" mid-value ONLY after semicolons (list continuation).
      //    Never strip after "." — that starts a new sentence and the subject is required.
      v = v.replace(MID_SEMICOLON_REPEAT, "; ");

      // 5. Replace any remaining wrong subjects (caregiver/caregivers) anywhere in the value
      if (ANY_WRONG_SUBJECT.test(v)) {
        console.log(`[NCP-DSHS-CLEAN] ⚠️  Field "${key}": residual "Caregiver will" mid-value — removing`);
        ANY_WRONG_SUBJECT.lastIndex = 0;
        v = v.replace(ANY_WRONG_SUBJECT, "");
      }

      // 6. If only "Staff will" remains with nothing after it, clear the field
      if (/^Staff\s+(?:will|to)\s*\.?$/.test(v.trim())) {
        console.log(`[NCP-DSHS-CLEAN] ⚠️  Field "${key}": only "Staff will" left after cleaning — clearing`);
        v = "";
      }
    } else {
      // For non-staff fields: replace "Caregiver will/Caregivers will" → "Staff will"
      if (/\b(?:Caregiver[s]?)\s+(?:will|to|must)\s+/i.test(v)) {
        console.log(`[NCP-DSHS-CLEAN] ⚠️  Field "${key}" (non-staff): contains "Caregiver will" — replacing`);
        v = v.replace(/\b(?:Caregiver[s]?)\s+(?:will|to|must)\s+/gi, "Staff will ");
      }
      // Replace "They will" when referring to staff (common AI pattern)
      v = v.replace(/\bThey\s+will\s+/g, "Staff will ");
    }
  ...
  console.log(`[NCP-DSHS-CLEAN] ✅ Cleanup complete — ${fixedCount} fields fixed (${caregiverWillCount} "Caregiver will" starts, ${duplicateStaffCount} duplicate "Staff will" collapsed)`);
  return data;
}
...
function getFieldTypeHint(name) {
  if (name.endsWith("_caregiver_actions") || name.endsWith("_assistance")) return "STAFF ACTION — must begin with 'Staff will...' or 'Staff to...' ONCE; all clauses joined with semicolons from that single subject — never repeat 'Staff will' mid-value and never use 'Caregiver will', 'Caregivers will', or any other subject";
...
━━━ CORE LANGUAGE RULES ━━━
1. VOICE: Third person only — "Resident", "Staff". Never "I", "you", "he", "she", or "they" as a substitute for the resident's name
2. TENSE: Present tense — "Resident ambulates...", "Staff provides..."
...
7. SUBJECT CONSISTENCY: NEVER use "Caregiver will", "Caregiver to", "Caregivers will", or any subject other than "Staff" in any field value. Always use "Staff will" or "Staff to". No exceptions.

━━━ FIELD-TYPE RULES ━━━

STAFF ACTION fields (ending in _caregiver_actions or _assistance):
  → MUST begin with "Staff will..." or "Staff to..."
  → Use "Staff will" ONCE at the start — never again in the same value
  → Do NOT use "Caregiver will", "Caregivers will", or any other subject after the opening
  → All subsequent clauses continue from the opening subject using semicolons: "Staff will do X; monitor Y; report Z."
  → BAD:  "Staff will help resident. Caregiver will assist with transfers. Caregivers will monitor."
  → BAD:  "Staff will help. Staff will also monitor."
  → GOOD: "Staff will provide hands-on assistance with transfers; monitor for safety; report changes to the nurse."
...
/** Strip any leading subject prefix from a raw value for staff action fields.
 * Prevents the AI from echoing "Staff will X" back as "Staff will Staff will X".
 */
const SUBJECT_PREFIX_STRIP = /^\s*(?:(?:Staff|Caregiver[s]?)(?:\s+or\s+\S+)*(?:\s+members?)?\s+(?:will|to|must|should|can)\s+|They\s+will\s+)/i;

function stripSubjectPrefix(value) {
  return value.replace(SUBJECT_PREFIX_STRIP, "");
}
...
function createLayer1UserPrompt(fieldBatch, extractedData, schema) {
  const fieldDetails = fieldBatch
    .map((name) => {
      ...
      // For caregiver action fields: strip any existing subject prefix from the raw value
      // so the AI writes its own clean "Staff will ..." without echoing the old prefix.
      const isCaregiverField = name.endsWith("_caregiver_actions") || name.endsWith("_assistance") || name.startsWith("caregiver_");
      const displayValue = (isCaregiverField && rawValue.length > 0) ? stripSubjectPrefix(rawValue) : rawValue;
...
async function generateLayer1Draft(extractedData, schema, extractionId) {
  ...
  cleanCaregiverFields(draft, schema);
...
1. VOICE & TENSE: All narrative fields in third person, present tense ("Resident demonstrates...", "Staff provides..."). Flag and fix any first-person, second-person, or past-tense language.

2. STAFF ACTIONS: Every _caregiver_actions and _assistance field must begin with "Staff will..." or "Staff to..." exactly once. All subsequent clauses must continue with semicolons from that single subject — never use "Caregivers will", "Caregiver will", or repeat "Staff will" mid-value. Fix any that violate this.

3. SUBJECT CONSISTENCY (applies to ALL fields): NEVER use "Caregiver will", "Caregiver to", "Caregivers will", or any subject other than "Staff" in any field value. If the current draft contains "Caregiver will", rewrite it as "Staff will". No exceptions.

4. STRENGTHS: Every _strengths_abilities field must describe resident capabilities positively and specifically. Must never describe deficits. Fix if field describes what the resident cannot do.

5. FREQUENCY: Every _how_often field must use measurable, clinical frequency. Replace vague terms:
...
6. SPECIFICITY: Eliminate all vague or informal language:
...
7. CROSS-SECTION CONSISTENCY: Use the RESIDENT CONTEXT block (provided in each prompt) to check for consistency. If one section establishes a condition (wheelchair use, dementia, vision impairment), other sections must reflect it accurately. Fix inconsistencies.

8. COMPLETENESS: Fields with actual content should have 1–3 substantive sentences minimum. Fix single-word or fragment answers.

9. CLINICAL VOCABULARY: Replace informal with clinical:
   - walks → ambulates
   - uses → utilizes
   - needs → requires
   - shows → demonstrates
...
10. NO FABRICATION: Do not add clinical details not present in the draft. If information is insufficient, preserve the existing content rather than inventing details.

11. PRESERVE FACTS: Keep all names, dates, diagnoses, medication names, frequencies, and phone numbers exactly as they appear.`;
...
async function generateLayer2Final(layer1Draft, schema, requiresCbhs, cbhsNotes, extractionId) {
  ...
  cleanCaregiverFields(final, schema);
