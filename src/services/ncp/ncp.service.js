...
async function generateNcpDocx(extractionId, user) {
  try {
    ...
    // If a DOCX already existed this is a regeneration — bump version and clear prior approval
    ...
    const updated = await prisma.ncpExtraction.update({
      ...
      data: {
        ...
        // Clear prior approval — new DOCX needs fresh review
        ...(isRegeneration ? { approvedAt: null, approvedBy: null, approvedByName: null } : {}),
      },
    });
 ...
 * @returns {Promise<Object>} Deleted extraction record
 */
async function deleteNcpExtraction(extractionId, tenantId) {
  try {
    console.log(`[NCP] Deleting extraction ${extractionId}...`);

    // Get extraction record first
    const extraction = await prisma.ncpExtraction.findFirst({
      where: {
        id: extractionId,
    ...
    // Delete source PDF from S3
    if (extraction.sourcePdfS3Key) {
      try {
        console.log(
          `[NCP] Deleting source PDF from S3: ${extraction.sourcePdfS3Key}`
        );
        const deletePdfCommand = new DeleteObjectCommand({
          Bucket: process.env.S3_BUCKET_NAME,
          Key: extraction.sourcePdfS3Key,
        });
        await s3Client.send(deletePdfCommand);
        console.log(`[NCP] ✅ Deleted source PDF from S3`);
      } catch (s3Error) {
        ...
        if (
          ...
          console.log(`[NCP] Source PDF not found in S3, skipping deletion`);
        } else {
          console.error(`[NCP] Error deleting source PDF from S3:`, s3Error);
          // Continue with deletion even if S3 deletion fails
        }
      }
    }

    // Delete populated DOCX from S3 (if exists)
    if (extraction.populatedDocxS3Key) {
      try {
        console.log(
          `[NCP] Deleting populated DOCX from S3: ${extraction.populatedDocxS3Key}`
        );
        const deleteDocxCommand = new DeleteObjectCommand({
          Bucket: process.env.S3_BUCKET_NAME,
          Key: extraction.populatedDocxS3Key,
        });
        await s3Client.send(deleteDocxCommand);
        console.log(`[NCP] ✅ Deleted populated DOCX from S3`);
      } catch (s3Error) {
        // Log error but don't fail if file doesn't exist
        if (
          s3Error.name === "NoSuchKey" ||
          ...
          console.log(
            `[NCP] Populated DOCX not found in S3, skipping deletion`
          );
        } else {
          console.error(
            `[NCP] Error deleting populated DOCX from S3:`,
            s3Error
          );
          // Continue with deletion even if S3 deletion fails
        }
    ...
    // Delete extraction record from database
    const deletedExtraction = await prisma.ncpExtraction.delete({
      where: { id: extractionId },
    });

    console.log(`[NCP] ✅ Successfully deleted extraction ${extractionId}`);

    return deletedExtraction;
  } catch (error) {
    console.error("[NCP] Error deleting extraction:", error);
    throw error;
  }
}
