...
async function listStaffMembers(tenantId, options = {}) {
  ...
  if (employmentStatus) {
    const status = String(employmentStatus).toUpperCase();
...
async function updateStaffProfile(tenantId, staffId, payload) {
  const staffRow = await getStaffMemberById(tenantId, staffId);
  const linkedUserId = staffRow.userId;
  ...
  if (payload.dateOfBirth !== undefined) {
    userData.dateOfBirth = parseOptionalDate(payload.dateOfBirth);
  }
