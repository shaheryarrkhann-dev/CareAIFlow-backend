...
const documentComplianceEvaluation = require("../compliance/document-compliance-evaluation.service");
const {
...
const STATUS_RANK = { COMPLIANT: 1, EXPIRING_SOON: 2, NON_COMPLIANT: 3 };

function pickStricterStatus(a, b) {
  return STATUS_RANK[a] >= STATUS_RANK[b] ? a : b;
}

function collectDocumentExpiryDetails(documents) {
  const now = new Date();
  ...
  return { status, expired, expiringSoon };
}

/**
 * Map document-compliance evaluation to staff list/dashboard status.
 * Aligns with OverallComplianceBadge on the staff documents page.
 */
function mapEvaluationToStaffListStatus(evaluation) {
  if (!evaluation) return "NON_COMPLIANT";
  const { overallStatus, readiness } = evaluation;
  if (overallStatus === "COMPLETE" && readiness === "READY") {
    return "COMPLIANT";
  }
  if (overallStatus === "MISSING" || readiness === "NOT_READY") {
    return "NON_COMPLIANT";
  }
  return "EXPIRING_SOON";
}

async function resolveStaffDocumentComplianceStatus(tenantId, staffId) {
  const documents = await prisma.staffDocument.findMany({
    where: { staffId },
    select: { id: true, fileName: true, expirationDate: true },
  });
  const expiry = collectDocumentExpiryDetails(documents);

  let evaluation = null;
  try {
    evaluation = await documentComplianceEvaluation.evaluateStaff(tenantId, staffId);
  } catch {
    evaluation = null;
  }

  const status = pickStricterStatus(
    mapEvaluationToStaffListStatus(evaluation),
    expiry.status
  );

  return { status, expired: expiry.expired, expiringSoon: expiry.expiringSoon };
}

/**
 * Get compliance status for a staff member (folder catalog + expiry dates).
 * @param {string} tenantId - Tenant ID
 * @param {string} staffId - Staff member ID
...
async function getComplianceStatus(tenantId, staffId) {
  await staffMemberService.getStaffMemberById(tenantId, staffId);
  const { status, expired, expiringSoon } =
    await resolveStaffDocumentComplianceStatus(tenantId, staffId);
...
async function getComplianceSummary(tenantId, restrictToStaffMemberId = null) {
  ...
  const summary = await Promise.all(
    staffMembers.map(async (staff) => {
      const { status, expired, expiringSoon } =
        await resolveStaffDocumentComplianceStatus(tenantId, staff.id);
