...
/**
 * Create standard root folders for a resident (idempotent by name at root).
 * Staff/facility already do this; residents previously had no auto-provision on live.
 * @param {string} residentId
 */
async function ensureDefaultResidentFolders(residentId) {
  for (const { name } of DEFAULT_RESIDENT_ROOT_FOLDERS) {
    const exists = await prisma.residentFolder.findFirst({
      where: { residentId, parentId: null, name },
    });
    if (!exists) {
      await prisma.residentFolder.create({
        data: {
          residentId,
          parentId: null,
          name,
...
async function getFolders(tenantId, residentId, parentId, user) {
  ...
  if (isRoot) {
    const rootCount = await prisma.residentFolder.count({
      where: { residentId, parentId: null },
    });
    if (rootCount === 0) {
      await ensureDefaultResidentFolders(residentId);
    }
  }
