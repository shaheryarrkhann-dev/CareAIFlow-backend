 ...
 * @param {string} [params.userRole] - Role for SUPER_ADMIN bypass
 ...
 */
async function createResident({ tenantId, userId, userRole, residentData }) {
  const facilityIdRaw =
    residentData?.facilityId || residentData?.facility_id || null;
  // Not a resident_fields.json key — strip before field mapping
  if (residentData && typeof residentData === "object") {
    delete residentData.facilityId;
    delete residentData.facility_id;
  }

  let resolvedFacility = null;
  try {
    resolvedFacility =
      await subscriptionManagement.resolveFacilityForResidentCap(
        tenantId,
        facilityIdRaw,
      );
  } catch (err) {
    // Super Admin may create without a facility when tenant has none yet
    if (userRole !== "SUPER_ADMIN") throw err;
  }

  await subscriptionManagement.assertCanAddResident(tenantId, {
    userId,
    userRole,
    facilityId: resolvedFacility?.id || facilityIdRaw,
  });
  ...
  if (resolvedFacility?.id) {
    prismaData.facilityId = resolvedFacility.id;
  }
  ...
  await residentFolderService.ensureDefaultResidentFolders(resident.id);
