...
async function inviteUser({
  ...
  const isAdminOfTenant =
    inviter.role === "ADMIN" && inviter.tenantId === tenantId;
  ...
  if (defaultRoleId) {
    ...
    if (defaultRole.isSystemTemplate) {
      throw new Error(
        "Cannot use a system template as default role; use a role from your organization",
      );
    }
    if (defaultRole.tenantId !== tenantId) {
      throw new Error(
  ...
  if (defaultRoleId) {
    ...
    if (
      customRole &&
      !customRole.isSystemTemplate &&
      customRole.tenantId === tenantId
    ) {
