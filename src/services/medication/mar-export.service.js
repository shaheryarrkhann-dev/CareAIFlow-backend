...
const { getMedications, getMedicationById } = require("./medication.service");
...
async function exportMarToPdf(user, filters = {}) {
  ...
  const { font, boldFont } = letterheadAssets.fonts;
  ...
  // Match progress notes PDF: small gap below letterhead before main title.
  currentY -= 10;
...
async function exportMedicationListToPdf(user, filters = {}) {
  ...
  // Add extra gap below letterhead before title.
  currentY -= 14;
  // Modern header without background
...
/**
 * Export a single medication profile to PDF (CareConnect letterhead).
 * @param {Object} user - Current user
 * @param {string} medicationId - Medication UUID
 * @returns {Promise<Buffer>}
 */
async function exportMedicationDetailToPdf(user, medicationId) {
  const med = await getMedicationById(medicationId, user);
  ...
  const drawLabelValue = (label, value, labelWidth = 140) => {
    const text =
    ...
    const wrapped = wrapText({
      text,
      maxWidth: contentWidth - labelWidth - 16,
      font,
      fontSize: 10,
    });
    const lineHeight = 14;
    const valueTopY = currentY;
    wrapped.forEach((line, idx) => {
      checkNewPage(lineHeight + 8);
      currentPage.drawText(line, {
        x: margin + labelWidth,
        y: valueTopY - idx * lineHeight,
        size: 10,
        font,
        color: rgb(0.15, 0.15, 0.15),
      });
    });
    currentY = valueTopY - wrapped.length * lineHeight - 12;
  };

  currentY -= 14;
  currentPage.drawText("Medication profile", {
    x: margin,
    y: currentY,
    size: 20,
    font: boldFont,
  ...
  const tenantName = med.tenant?.name || "Facility";
  const generatedAt = new Date().toLocaleString("en-US", {
    month: "short",
    day: "numeric",
    year: "numeric",
  ...
  drawLabelValue("Medication", med.name || "—");
  drawLabelValue(
    "Resident",
    med.residentName || med.residentId || "—"
  );
  drawLabelValue("Dosage", med.dosage || "—");
  drawLabelValue("Route", med.route || "—");
  drawLabelValue("Frequency", med.frequency || (med.isPrn ? "PRN" : "—"));
  drawLabelValue(
    "Type",
    med.isPrn ? "PRN (as needed)" : "Scheduled"
  );
  drawLabelValue("Status", med.isActive ? "Active" : "Inactive");
  drawLabelValue(
    "Start date",
  ...
  if (med.timeSlots && med.timeSlots.length > 0) {
    drawLabelValue("Scheduled times", med.timeSlots.join(", "));
  }

  drawLabelValue("Prescriber", med.prescriberName || "—");
  drawLabelValue("Prescriber phone", med.prescriberPhone || "—");
  drawLabelValue("Pharmacy", med.pharmacyName || "—");
  drawLabelValue("Pharmacy phone", med.pharmacyPhone || "—");
  drawLabelValue("Special instructions", med.specialInstructions || "—");
  drawLabelValue("Requires vitals", med.requiresVitals ? "Yes" : "No");
  if (med.vitalsType) {
    drawLabelValue("Vitals type", med.vitalsType);
  }

  const counts = med._count;
  if (counts) {
    drawLabelValue(
      "Related records (approx.)",
      `MAR: ${counts.marRecords ?? 0}, PRN: ${counts.prnRecords ?? 0}, Schedules: ${counts.schedules ?? 0}`
    );
  }
