...
async function exportCarePlanToPdf(carePlanId, options = {}, requestingUser) {
  ...
  // Prepare letterhead assets (logo, fonts, facility info)
  const letterheadAssets = await prepareLetterheadAssets(
    pdfDoc,
    carePlan.tenantId,
    carePlan.facilityId || null,
  ).catch(() => null);

  // Add first page with letterhead
  const page = letterheadAssets
    ? addLetterheadPage(pdfDoc, letterheadAssets)
    : pdfDoc.addPage([612, 792]);

  const font = letterheadAssets?.fonts?.font || await pdfDoc.embedFont(StandardFonts.Helvetica);
  const boldFont = letterheadAssets?.fonts?.boldFont || await pdfDoc.embedFont(StandardFonts.HelveticaBold);

  // Page dimensions
  const pageWidth = page.getWidth();
  const pageHeight = page.getHeight();
  const margin = letterheadAssets?.bodyMargin ?? 50;
  const sectionSpacing = 20;
  const lineHeight = 15;

  // Track current Y position — start below letterhead header
  let currentY = letterheadAssets?.contentStartY ?? (pageHeight - margin);
  let currentPage = page;

  // Helper function to add new page if needed
  const checkNewPage = (requiredSpace) => {
    if (currentY - requiredSpace < (letterheadAssets?.letterheadFooterHeight ?? 50) + margin) {
      const newPage = letterheadAssets
        ? addLetterheadPage(pdfDoc, letterheadAssets)
        : pdfDoc.addPage([612, 792]);
      currentY = letterheadAssets?.contentStartY ?? (pageHeight - margin);
      currentPage = newPage;
      return newPage;
    }
    return currentPage;
  ...
  // Get tenant name
  const tenantName = letterheadAssets?.context?.displayName || carePlan.tenant?.name || "Unknown Facility";
  ...
  if (letterheadAssets) {
    stampPageNumbers(pdfDoc, letterheadAssets.fonts.font, margin);
  }
