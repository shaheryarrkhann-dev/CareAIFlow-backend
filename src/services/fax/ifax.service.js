...
async function sendFaxReport({
  ...
  // iFax total request ~20MB; base64 expands ~4/3 — keep decoded PDF under ~14MB
  const maxBytes = 14 * 1024 * 1024;
  if (pdfBuf.length > maxBytes) {
    throw new Error(
      `PDF is too large for fax (max about ${Math.round(maxBytes / (1024 * 1024))}MB).`
    );
  }
  ...
  const payload = {
    ...
    faxData: [{ fileName: safeName, fileData }],
  };
