...
/**
 * Resolve tenantId for role operations.
 * SUPER_ADMIN can pass tenantId in body/query.
 * Other users: optional explicit tenantId must be a tenant they belong to (primary or UserTenant).
 * @param {{ id: string, tenantId?: string|null, role?: string }} user - req.user
 * @param {{ tenantId?: string }} bodyOrQuery
 * @returns {Promise<string|null>}
 */
async function resolveTenantId(user, bodyOrQuery = {}) {
  const requested = bodyOrQuery.tenantId || null;
  if (user.role === "SUPER_ADMIN") {
    return requested || user.tenantId || null;
  }
...
async function getTemplates() {
  return prisma.customRole.findMany({
    where: { isSystemTemplate: true, tenantId: null, isActive: true },
    orderBy: { name: "asc" },
    include: {
      rolePermissions: { include: { permission: true } },
