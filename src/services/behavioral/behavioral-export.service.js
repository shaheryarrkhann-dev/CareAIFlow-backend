...
const WSA_LOGO_PATH = path.join(__dirname, "../../../assets/WSA-logo.png");

/**
 * Convert a logo PNG to solid black while preserving anti-aliased edges.
 * Handles logos without an alpha channel (white background keyed to transparent).
 * @param {Buffer} imageBuffer
 * @returns {Promise<Buffer>}
 */
async function toSolidBlackLogoPng(imageBuffer) {
  ...
  for (let i = 0; i < info.width * info.height; i++) {
    ...
    if (lum >= 250) {
      ...
      out[idx + 3] = Math.min(255, Math.round((255 - lum) * 1.8));
    }
  }
...
async function exportBehavioralReportToPdf(user, filters = {}) {
  ...
  // Create new PDF document (universal CareAIFlow letterhead on every page)
  const pdfDoc = await PDFDocument.create();
  const font = await pdfDoc.embedFont(StandardFonts.Helvetica);
  const boldFont = await pdfDoc.embedFont(StandardFonts.HelveticaBold);
  const semiBoldFont = boldFont;
  const tenantId = logs[0]?.tenantId || user.tenantId;
  const facilityId = logs[0]?.facilityId || filters.facilityId || null;
  const letterheadAssets = await prepareLetterheadAssets(
    pdfDoc,
    tenantId,
    facilityId
  );
  const page = addLetterheadPage(pdfDoc, letterheadAssets);

  // Page dimensions
  const pageWidth = page.getWidth();
  const pageHeight = page.getHeight();
  const margin = 36;
  const footerMargin = contentBottomMin(letterheadAssets);
  const sectionSpacing = 20;
  const contentStartY = letterheadAssets.contentStartY;

  // Track current Y position from top content area
  ...
  const checkNewPage = (requiredSpace) => {
    if (currentY - requiredSpace < footerMargin) {
      const newPage = addLetterheadPage(pdfDoc, letterheadAssets);
  ...
  // ============================================
  // FOOTER ON ALL PAGES (CBHS report only)
  // ============================================
  ...
  for (let index = 0; index < pages.length; index++) {
    ...
    const pageNumberWidth = font.widthOfTextAtSize(pageNumberText, 7);
  ...
  try {
    ...
    if (isFontBufferError && typeof isDancingScriptLoaded !== "undefined" && isDancingScriptLoaded) {
...
async function generateBatchLogsPdf(
  ...
  // CBHS tracking form: plain pages (no universal letterhead — regulatory form layout only)
  const pdfDoc = await PDFDocument.create();
  ...
  const pageHeight = page.getHeight();
  const margin = 36;
  const footerMargin = contentBottomMin(null);
  const sectionSpacing = 20;
  /** First body baseline from top when letterhead is omitted (matches ~50pt top inset). */
  const contentStartY = pageHeight - 50;
  ...
  const titleBlockWidth = Math.max(
    semiBoldFont.widthOfTextAtSize(titleLine1, titleFontSize),
    semiBoldFont.widthOfTextAtSize(titleLine2, titleFontSize)
  );
  const headerGap = 16;

  if (fs.existsSync(WSA_LOGO_PATH)) {
    try {
      const logoBytes = await toSolidBlackLogoPng(
        fs.readFileSync(WSA_LOGO_PATH)
      );
      const logoImage = await pdfDoc.embedPng(logoBytes);
      const availableLogoWidth =
        pageWidth - margin * 2 - titleBlockWidth - headerGap;
      const maxLogoHeight = 44;
      let logoScale = Math.min(
        maxLogoHeight / logoImage.height,
        availableLogoWidth / logoImage.width
      );
      logoScale *= 0.85;
      const logoWidth = logoImage.width * logoScale;
      const logoHeight = logoImage.height * logoScale;
      const logoX = pageWidth - margin - logoWidth;
      const headerBandTop = headerY + 18;
      const headerCenterY = (headerBandTop + ruleY) / 2;
      ...
      console.warn("[PDF] Could not embed WSA logo:", logoErr.message);
    }
  }
