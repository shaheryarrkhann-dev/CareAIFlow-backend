...
const { requirePlanModule } = require("../../middlewares/plan-entitlement.middleware");
...
router.post(
  "/pdf",
  requirePlanModule("aiAdmissions"),
...
/**
 * GET /api/residents/capacity-status
 * Active count vs facility capacity (disable create when at limit).
 * Must be registered before /:id.
 */
router.get(
  "/capacity-status",
  residentController.getResidentCapacityStatus
);

/**
 * POST /api/residents/export/roster
 * Export resident roster to PDF using universal letterhead service
 */
router.post(
  "/export/roster",
  enforceTenantIsolation("tenantId"),
  residentController.exportResidentRoster
);
...
// NOTE: POST /:id/esignature removed — e-signatures are now handled via DocuSign (/api/docusign/envelopes)

/**
 * PATCH /api/residents/:id/status
 * Set resident status to Active or Inactive (admin only)
 */
router.patch(
  "/:id/status",
  requireAdminOrSuperAdminForDelete(),
  validate(updateResidentStatusValidator),
