...
-- Add new audit actions to AuditAction enum (type created in a later baseline migration in this repo)
DO $$ BEGIN
...
-- AddForeignKey (guarded: tenants/users baseline runs later in this repo)
DO $$
BEGIN
...
DO $$
BEGIN
  IF to_regclass('public.care_plan_problems') IS NOT NULL AND to_regclass('public.care_plans') IS NOT NULL THEN
    BEGIN
    ...
    END;
  END IF;
  IF to_regclass('public.care_plan_goals') IS NOT NULL AND to_regclass('public.care_plan_problems') IS NOT NULL THEN
    BEGIN
    ...
    END;
  END IF;
  IF to_regclass('public.care_plan_interventions') IS NOT NULL AND to_regclass('public.care_plan_goals') IS NOT NULL THEN
    BEGIN
...
DO $$
BEGIN
  IF to_regclass('public.care_plan_versions') IS NULL THEN
    RETURN;
  END IF;
...
DO $$
BEGIN
  IF to_regclass('public.care_library') IS NULL THEN
    RETURN;
  END IF;
  IF to_regclass('public.tenants') IS NOT NULL THEN
    BEGIN
...
DO $$
BEGIN
  IF to_regclass('public.care_plan_alerts') IS NULL OR to_regclass('public.care_plans') IS NULL THEN
    RETURN;
  END IF;
  BEGIN
