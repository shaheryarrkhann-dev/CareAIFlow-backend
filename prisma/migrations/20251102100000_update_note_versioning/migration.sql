...
-- `residentName` was introduced in a later superseded migration; keep compatible with shadow replay.
ALTER TABLE "notes" ADD COLUMN IF NOT EXISTS "residentName" TEXT;
...
DO $$
BEGIN
  IF to_regclass('public.note_versions') IS NULL THEN
    RETURN;
  END IF;
  BEGIN
    ALTER TABLE "note_versions"
